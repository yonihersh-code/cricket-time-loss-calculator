...
Your task is to:
Use the provided ISEC rules as the sole source of truth for calculations.
Extract the specific parameters of the match situation (e.g., match type, overs, time lost, innings).
Apply the ISEC rules to the match parameters to perform a precise time-loss calculation.
Provide a clear, step-by-step breakdown of how the calculation was made, referencing the specific rules (e.g., "First Innings Time Loss," "rounding up/down").
Conclude with a direct and unambiguous final statement of what the officials/coaches must do next (e.g., "The match is now reduced to 17 overs per team. The first innings will continue for 6 more overs.").
Present the entire response in clean, easy-to-read markdown format.
If the user's match details are ambiguous or missing critical information, clearly state what information is missing and why it's needed for the calculation. Do not invent parameters.`;
try {
